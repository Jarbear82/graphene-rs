...
use graphene_core::{EdgeData, GraphState, Size2, Vec2, NodeId};
use graphene_layout::{
    CircleLayout, ForceDirectedLayout, CoseLayout, Layout,
    ...
    compute_hierarchical_edge_bundling, compute_multigraph_bezier_routing
};
...
fn main() {
    ...
    println!("  Circle: Node 1 at ({:.2}, {:.2})", state.positions.get(0).x, state.positions.get(0).y);
    ...
    println!("  Force: Node 1 at ({:.2}, {:.2})", state.positions.get(0).x, state.positions.get(0).y);

    // 4b. Compute CoSE Layout
    println!("\n--- Computing CoseLayout (Compound Spring Embedder) ---");
    let mut cose = CoseLayout::default();
    cose.compute(&mut state);
    println!("  CoSE: Node 1 at ({:.2}, {:.2})", state.positions.get(0).x, state.positions.get(0).y);
    ...
    println!("  Kamada-Kawai: Node 1 at ({:.2}, {:.2})", state.positions.get(0).x, state.positions.get(0).y);
    ...
    println!("  Sugiyama: Node 1 at ({:.2}, {:.2})", state.positions.get(0).x, state.positions.get(0).y);
    ...
    println!("  Reingold-Tilford: Node 1 at ({:.2}, {:.2})", state.positions.get(0).x, state.positions.get(0).y);

    // 8. Compute MDS Layout
    println!("\n--- Computing MdsLayout (Multidimensional Scaling) ---");
    let mut mds = MdsLayout::default();
    mds.compute(&mut state);
    println!("  MDS: Node 1 at ({:.2}, {:.2})", state.positions.get(0).x, state.positions.get(0).y);
    ...
    println!("  Grid Sorted: Node 1 at ({:.2}, {:.2})", state.positions.get(0).x, state.positions.get(0).y);
    ...
    println!("  Concentric Hub: Node 1 at ({:.2}, {:.2})", state.positions.get(0).x, state.positions.get(0).y);

    // 11. Compute Bipartite Column Layout
    println!("\n--- Computing BipartiteLayout ---");
    let node_indices: HashMap<NodeId, usize> = state.node_index_to_id.iter()
        .enumerate()
        .map(|(idx, &id)| (id, idx))
        .collect();
    let mut bipartite = BipartiteLayout {
        partition_fn: move |id: NodeId| if *node_indices.get(&id).unwrap_or(&0) % 2 == 0 { 0 } else { 1 },
        column_spacing: 120.0,
        vertical_spacing: 50.0,
    };
    bipartite.compute(&mut state);
    println!("  Bipartite: Node 1 at ({:.2}, {:.2})", state.positions.get(0).x, state.positions.get(0).y);
    ...
    println!("  Weighted Force: Node 1 at ({:.2}, {:.2})", state.positions.get(0).x, state.positions.get(0).y);
    ...
    println!("  Collision Force: Node 1 at ({:.2}, {:.2})", state.positions.get(0).x, state.positions.get(0).y);
    ...
    println!("  Dijkstra destination nodes calculated: {}", distances.len());
